...
void StudentRecords::report_card(){
    ...
    else
    {
        ...
        {
            int sid = stu.get_id();
            outFile << "Report Card for " << get_student_name(sid) << endl;
            for (Grade& grd : grades)
                if (grd.get_student_id() == sid){
                    outFile << get_course_name(grd.get_course_id()) << ": " << grd.get_grade() << endl;
                    unsigned char current_credits = get_course_credits(grd.get_course_id());
                    credits += current_credits;
                    points += get_num_grade(grd.get_grade()) * current_credits;
                }
            outFile << "GPA: " << (points / credits) << endl;
            outFile << "=======================================" << endl;
        }
        cout << "Report_card.txt successfully generated" << endl;
        outFile.close();
    }
